...
public class AdminUserResponse {
    ...
    Instant activePlanExpiresAt;
    boolean twoFactorEnabled;

    public static AdminUserResponse from(User user, Long effectiveStorageQuotaBytes, long storageUsedBytes) {
        return from(user, effectiveStorageQuotaBytes, storageUsedBytes, null, null);
    }

    public static AdminUserResponse from(User user, Long effectiveStorageQuotaBytes, long storageUsedBytes,
            String activePlanName, Instant activePlanExpiresAt) {
        return new AdminUserResponse(user.getId(), user.getEmail(), user.getFullName(), user.isEnabled(),
                ...
                activePlanName, activePlanExpiresAt, user.isTwoFactorEnabled());
    }
}
